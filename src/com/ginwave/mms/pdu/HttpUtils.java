...
public class HttpUtils {
	private static final String TAG = "Mms.HttpUtils";
	...
	public static byte[] httpConnection(Context context, long token,
		...
		/*int inetAddr = lookupHost(proxyHost);
		...
		try {
			...
			if (status.getStatusCode() != 200) { // HTTP 200 is success.
				throw new IOException("HTTP error: " + status.getStatusCode() + " "+  status.getReasonPhrase());
			}
					...
					if (entity.isChunked()) {
						...
						try {
							...
							if (bytesRead == -1 && offset > 0 && !readError) {
								...
								MobclickAgent.onEvent(context, SetDataSource.MMSCOUNT);
								Log.v(TAG,
	...
	private static AndroidHttpClient createHttpClient(Context context) {
		...
		if (Log.isLoggable(TAG, Log.DEBUG)) {
			Log.d(TAG, "[HttpUtils] createHttpClient w/ socket timeout "
